    ...
    <form class="flex flex-col gap-3 space-y-1 pt-8" @submit.prevent="login">
      <p class="flex flex-row items-center justify-center">
        <input type="text" placeholder="email" v-model="email"
          class="rounded-3xl w-[80%] p-3 mt-2 border-2 border-solid border-blue-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-200 focus:outline-none" />
      </p>
      ...
      <p class="flex items-center justify-center">
        <button type="submit" placeholder="password"
          class="rounded-md w-[80%] p-3 mt-2 bg-blue-500 text-white uppercase active:bg-blue-400">Login</button>
      </p>
    </form>
  </div>
</template>
