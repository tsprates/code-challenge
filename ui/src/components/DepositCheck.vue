<template>
    ...
    <div class="max-w-screen-md mx-auto">
        <form @submit.prevent="depositCheck">
            ...
            <!-- preview -->
            <div class="border-dashed border-4 border-blue-200 mx-4 h-32 flex flex-col items-center justify-center"
                v-if="picture.data === null">
                <label for="picture">
                    <span class="text-blue-300 flex flex-col items-center justify-center active:text-blue-200">
            ...
            <div class="fixed bottom-2 left-0 md:relative md:bottom-1 md:mt-5 p-2 w-full">
                <button type="submit" :disabled="submitting" :class="{ button: true, 'bg-blue-200': submitting }">Deposit Check</button>
            </div>
        </form>
    </div>
</template>
...
const picture = reactive({ file: null, data: null })
